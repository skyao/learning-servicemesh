...
需要提醒的是，虽然Service Mesh这个词汇直到2016年9才出现，但是和Service Mesh一脉相承的技术很早就出现了，经过了长期的发展和演变，才形成了今天的Service Mesh，并且这个演进的过程还在继续。

## 远古时代的案例

![](images/qcon-ppt-13.jpg)

远在微服务出现之前，在计算机的"远古时代"，在第一代网络计算机系统中，最初的一代开发人员需要在应用代码里处理网络通讯的细节问题，比如说数据包顺序、流量控制等等，导致网络通讯逻辑和业务逻辑混杂在一起。为了解决这个问题，TCP/IP技术，解决了流量控制问题，并将这些功能做成通用能力。从右边的图上可以看到，实际功能没发生变化：所有的功能都在，相应的代码也在。但是，最重要的事情，流程控制，已经从应用程序里面剥离出来。抽出来的这些功能被进一步标准化和统一化，最终被做成了操作系统网络层的一部分，这就是TCP/IP协议栈。
...
当时间流转，进入微服务时代后，我们也面临着类似的一些问题：比如说我们在做微服务的时候要处理一系列的比较基础和通用的事情，如服务发现，在得到服务器实例列表之后做负载均衡，为了保护服务器要熔断/重试等等。
...
和Service Mesh，尤其是新贵Istio相比，传统侵入式框架如Spring Cloud、Dubbo所能提供的功能是很有限的，尤其不像Isito这样形成一个非常完备的体系。典型如Content Based Routing和Version Based Version，Istio借助这两个特性可以实现非常强大而有弹性的服务治理功能，其能力远远超过Spring Cloud等。
...
微服务在面世时，承诺了一个很重要的特性：微服务可以采用最适合的语言来编写。理论上说，不同的团队，不同的微服务，可以根据实际情况选择团队最擅长，或者最适合当前应用的的编程语言。

但是，在实践中，这个承诺往往受到极大挑战而沦为空话：微服务的确理论上可以使用不同的语言，但是实际开发时，当需要选择通过框架或者类库来实现代码重用时，就会发现有个绕不开的问题——框架和类库是语言相关的！

这个问题非常尖锐，为了解决它，通常只有两个思路：

1. 统一编程语言，整个系统就只使用一种编程语言，只为它提供框架和类库。
2. 为要使用的每种编程语言都提供框架和类库

前者相当于放弃了微服务的跨语言特性，并且面临强行用一种编程语言完成所有不同类型工作的境地，同样会有很多麻烦。后者理论上可行，但是实践中会面临非常大的开发工作量，后期维护代价高昂，还有不同编程语言不同版本之间的兼容性问题，可谓代价极高，往往无法坚持。

### 痛点4：升级困难

![](images/qcon-ppt-19.jpg)

即使解决了多语言支持的问题，无论是统一为一个语言，还是开发多个语言版本，依然还会继续遇到下面这个问题：版本升级。
...
实际上做不到的。任何一家正规的公司，业务应用上线必然是有一个完备的流程，当应用程序发生变更，需要重新上线时，必然要走测试和上线的标准流程。类库升级，即使是在完全兼容不需要修改任何业务代码的前提下，也是需要业务应用重新打包发布的，因为"侵入式框架"是需要将框架的内容打包进去业务应用的！
...
1. 问题的根源在哪里？
	...
	答案是：没有。这些问题都属于服务间通讯的范围，而应用本身的实现逻辑无关。
...
4. 有什么内容是普适的？
	...
	前面遇到的这些问题具有高度的普适性：所有的语言、框架、组织，这些问题对于任何一个微服务都是同样存在的。
...
早在微服务出现之前，为了解决客户端和服务器端直接耦合的问题，之前有一些先驱者，尝试过使用Proxy的方案，来隔离客户端和服务器端。典型如Nginx，HAProxy，Apache等HTTP反向代理。这些Proxy和微服务没有联系，但是提供了一个基本思路：在服务器端和客户端之间插入了一层来完成请求转发的功能，避免两者直接通讯。所有流量都经由Proxy转发，而Proxy需要为此实现基本的特性如负载均衡。

当然这些Proxy的功能非常简陋，比如服务发现甚至是通过配置文件来实现的。功能不够，但是思路和想法是很有参考语义的：客户端和服务器端应该隔离，部分功能下沉到中间层来实现请求转发。Proxy模式的探索为日后Service Mesh的出现奠定了基础。
...
Sidecar模式借鉴了Proxy模式的思路，Sidecar扮演的角色和Proxy很类似，但是在功能实现上就齐全很多。基本思路是Sidecar对齐原来侵入式框架在客户端实现的各种功能，实现上通常是通过增加一个Proxy实现请求转发，然后直接重用原有的客户端类库。

**Sidecar模式和Proxy模式的差异在于功能点是否齐全**：

- Proxy只具备基本的简单功能
- Sidecar对齐侵入式框架的功能

这中思路下发展出来的Sidecar通常都是有局限性的，变现在**为特定的基础设施而设计**，通常是和开发Sidecar的公司当时的基础设施和框架直接绑定的，在原有体系上搭建出来，有这样的特殊的背景和需求。带来的问题就是这些Sidecar会有很多由原有体系带来的限制，导致无法通用，只能工作在原有体系中，无法对外推广。
...
另外还有一点，Sidecar通常是可选的，容许直连。通常是编写框架的编程语言的客户端维持原有的客户端直连方式，其他语言不开发客户端而选择走Sidecar转发的方式，当然也可以选择都走Sidecar。但是，在Service Mesh中，由于要求完全掌控所有流量，所以要求所有请求都必须通过Service Mesh转发，不提供直连方式。
